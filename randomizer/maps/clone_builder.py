...
class PlayerCloneContext:
    ...
    power_delivery_source_ids: set[str]
    controllable_delivery_source_ids: set[str]
...
def build_player_clone_sections(
    ...
    power_delivery_source_ids = context.power_delivery_source_ids
    controllable_delivery_source_ids = (
        context.controllable_delivery_source_ids
    )
    ...
    for unit_id, target_unit_id, counts in clone_candidates:
        ...
        power_delivery_clone = unit_id in power_delivery_source_ids
        payload_reference_clone = (
            initial_payload_clone or power_delivery_clone
        )
        ...
        if owned_template is not None or owned_clone_rule_overlays.get(unit_id):
            ...
            if (
                ...
                # can survive on the generated clone and expose the original after
        ...
        # Every actual player/spawn TechnoType clone must carry one complete,
        # positive health baseline. Building add-ons intentionally omit
        # Strength and exist only as PowersUpBuilding sidebar upgrades; adding
        # a fabricated Strength made valid Reloaded add-ons fail generation.
        is_building_addon = any(
            str(key).lower() == 'powersupbuilding'
            and str(value).strip()
            for key, value in clone_source_values.items()
        )
        if not is_building_addon:
            try:
                normalize_unit_strength(
                    clone_source_values,
                    target,
                ...
                raise ValueError(
                    f'{unit_id} (buff target {target_unit_id}): {exc}'
                ) from exc
        ...
        if unit_id in buildable_ids:
            ...
            if payload_reference_clone:
                # Carrier contents and power-delivered units must not inherit
                # native faction or prerequisite gates. Carrier-only support
                # remains unselectable; independent paradrop/drop-pod units
                # explicitly listed as controllable remain player-commandable.
                if owner_ids:
                ...
                if unit_id in controllable_delivery_source_ids:
                    clone_values['Selectable'] = 'yes'
                    clone_values['IsSelectableCombatant'] = 'yes'
                else:
    ...
    # stable root when a deploy/undeploy pair has different source IDs.
