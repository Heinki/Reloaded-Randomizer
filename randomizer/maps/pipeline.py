...
from randomizer.maps.rules import (
    ...
    validate_player_clone_house_isolation,
...
from randomizer.missions.catalogue import campaign_filter_factions
...
def prepare_hooked_map(self, mission, extra_rules=None):
    ...
    single_faction_campaign = bool(
        active_campaign_filter != 'All Campaigns'
        and len(campaign_filter_factions(active_campaign_filter)) == 1
    )
    fallback_tech_ids = {
        section.upper()
    ...
    authored_source_hash = file_sha256(integrity_path)
    lines = IniLines(read_text(source_path).splitlines())
    # Every authored action is part of the mission's runtime contract.  In
    # particular, Action 106 is also used by campaign scripts to stage units
    # such as the Terrorist and Demolition Truck.  Progression gating may
    # control player-production clones, but it must never delete or retarget
    # an action already present in the source map.
    authored_action_ids = frozenset(
        str(action_id).strip().lower()
        for action_id in section_value_map_preserve(lines, 'Actions')
    ...
    self.append_log(
        ...
        'action, and map-record rewrites.'
    )
    ...
    controllable_delivery_clone_ids = unique_in_order(
        unit_id
        for reward in canonical_rewards(launch_power_rewards)
    ...
    delivery_clone_ids = unique_in_order(
        controllable_delivery_clone_ids
        + [
            type_id.strip().upper()
    ...
    if delivery_clone_ids:
        self.append_log(
            'Prepared player-clone delivery payload IDs: '
            + ', '.join(delivery_clone_ids)
            + '.'
        )
    ...
    # fields. Unregistered generated sections enforce unearned access;
    # registered sections carry earned/mission production rules.
    ...
    # Generated clone rewrites; restoring every non-player-used native type erases
    ...
    if launch_active:
        ...
        (
            ...
            forced_delivery_clone_ids=delivery_clone_ids,
        ...
        native_starting_plan = starting_unit_buff_plan(
            lines,
            configured_helper_houses=reward_helpers,
            excluded_player_houses=excluded_player_houses,
        )
        if native_starting_plan.friendly_type_ids:
            self.append_log(
                'Starting-unit buff isolation: '
                f'{len(native_starting_plan.native_direct_type_ids)} native '
                'friendly-only type(s) eligible for guarded direct buffs; '
                f'{len(native_starting_plan.shared_type_ids)} type(s) shared '
                'with opponent starting forces remain native and unmodified.'
            )
        (
            ...
            native_starting_buff_ids=(
                native_starting_plan.native_direct_type_ids
            ),
    ...
    protected_action_ids = authored_action_ids | preserved_ai_action_ids
    rewritten_native_unlocks = rewrite_techlevel_actions(
        lines,
        mission_unlock_clone_replacements if launch_active else {},
        preserved_action_ids=protected_action_ids,
    ...
    if launch_active:
        ...
        clone_house_validation = validate_player_clone_house_isolation(
            lines,
            clone_handled,
            buffed_helper_houses=reward_helpers,
            denied_houses=configured_enemies,
            excluded_player_houses=excluded_player_houses,
        )
        if clone_house_validation['failures']:
            raise ValueError(
                'Generated player/helper clone leaked to a hostile or '
                'unreviewed House: '
                + '; '.join(clone_house_validation['failures'])
            )
        self.append_log(
            'Validated hostile-house isolation for '
            f'{clone_house_validation["checked"]} referenced player/'
            'helper clone(s); no hostile clone consumers found.'
        )
    ...
    identity_validation = validate_authored_identity_contract(
        identity_contract, lines
    )
    self.append_log(
        'Validated authored identity preservation: '
        f'{identity_validation["protected_type_count"]} type(s), '
        f'{identity_validation["positioned_reference_count"]} positioned '
        'reference(s), '
        f'{identity_validation["record_reference_count"]} authored record(s).'
    )
    ...
    generated_path = GENERATED_MAP_DIR / launch_scenario
    ...
    root_map = GAME_ROOT / launch_scenario
    if root_map.exists() and not is_generated_hooked_map(root_map):
        raise FileExistsError(
            f'Refusing to overwrite non-randomizer map: {root_map}'
        )
    ...
    return {
        ...
        'source_path': str(integrity_path),
        'source_sha256': authored_source_hash,
