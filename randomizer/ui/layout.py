...
from randomizer.ui.config import UNLOCK_DASHBOARD_FACTIONS
...
def _build_window_shell(self):
    ...
    header = ttk.Label(
        main_frame,
        text=f'{WINDOW_TITLE} v{APP_VERSION}',
...
def _build_right_panel(self, main_frame):
    ...
    self.campaign_label = ttk.Label(options_row, text='Campaign / Add-on')
    ...
    WidgetTooltip(
        ...
        (
            ...
            'Rewards per mission. Explicit campaign finales automatically '
            'grant the higher finale reward. Missions with more objectives '
            'do not produce more rewards.'
            if OBJECTIVE_REWARDS_READY else
            'Victory-only rewards are required until explicit objective '
            'Action mappings have been reviewed. Every mission has a reviewed '
            'victory hook; objective rewards remain disabled.'
        ),
    )
    ...
    ttk.Label(
        ...
        text=(
            ...
            'run. Shop Mode uses its own reward '
...
def _build_info_tabs(self, info_tabs):
    ...
    for faction in UNLOCK_DASHBOARD_FACTIONS:
