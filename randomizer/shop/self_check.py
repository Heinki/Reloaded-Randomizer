...
def _current_shop_feature_checks():
    ...
    roulette_key = 'RLR-SHOP-FACTION-ROULETTE:run-1'
...
def _phase_four_checks():
    mission_pool = [
        {
            ...
            'reward_class': (
                MissionEconomyClass.STANDARD.value
                if index < 18 else MissionEconomyClass.FINALE.value
            ),
        }
    ...
    with TemporaryDirectory(prefix='mo-shop-run-self-check-') as temporary:
        ...
        service.start_run(
            ...
            starting_unit_ids=('RLR_T1_INFANTRY',),
            starting_defense_ids=('RLR_T1_DEFENSES',),
        )
        total_meta_coins = 0
...
def _phase_five_checks():
    ...
    offers = (
        MissionOffer('SC_AP_1', MissionEconomyClass.STANDARD),
    )
...
def _phase_seven_checks():
    offers = (
        MissionOffer('SC_POLISH_1', MissionEconomyClass.STANDARD),
        MissionOffer('SC_POLISH_2', MissionEconomyClass.STANDARD),
...
def validate_shop_domain():
    ...
    standard = mission_reward(MissionEconomyClass.STANDARD)
    finale_with_bonus = mission_reward(
        MissionEconomyClass.FINALE, victory_coin_bonus_level=3
    )
    capped_bonus = mission_reward(
        MissionEconomyClass.FINALE, victory_coin_bonus_level=999
    )
    ...
    economy_valid = bool(
        (standard.run_coins, standard.meta_coins) == (3, 1)
        and finale_with_bonus.run_coins == 13
        ...
        and permanent_power_price('DROPPODSPECIAL') == 25
        and permanent_power_price('NUKESPECIAL') == 50
    ...
    catalogue_valid = bool(
        ...
        and all(
            ...
            for price in SHOP_CONFIG.power_target_prices.values()
        )
    ...
    permanent_snapshot = permanent_buff_snapshot(
        ...
        permanent_power_reward_ids=(permanent_power_purchase.validation.reward_id,),
    )
    ...
    mission_pool = [
        {
            'code': 'SC_STANDARD_HERO',
            'reward_class': 'standard',
            'true_no_build': True,
        },
        {'code': 'SC_STANDARD_1', 'reward_class': 'standard'},
        {'code': 'SC_STANDARD_2', 'reward_class': 'standard'},
        {'code': 'SC_STANDARD_3', 'reward_class': 'standard'},
        {'code': 'SC_STANDARD_4', 'reward_class': 'standard'},
        {'code': 'SC_STANDARD_5', 'reward_class': 'standard'},
        {'code': 'SC_STANDARD_6', 'reward_class': 'standard'},
        {'code': 'SC_STANDARD_7', 'reward_class': 'standard'},
        {'code': 'SC_STANDARD_8', 'reward_class': 'standard'},
        {'code': 'SC_STANDARD_9', 'reward_class': 'standard'},
    ...
    mission_rules_valid = bool(
        ...
        and 'SC_STANDARD_HERO' in {
            offer.mission_code for offer in offers
        }
        and mission_classes_for_stage(1) == {
            MissionEconomyClass.STANDARD
        }
        and mission_classes_for_stage(2) == {MissionEconomyClass.STANDARD}
        and all(
        ...
        and classify_mission({
            'code': 'SC_STANDARD', 'reward_class': 'standard'
        }) is MissionEconomyClass.STANDARD
        and classify_mission({
            'code': 'SC_FINALE', 'reward_class': 'finale'
        }) is MissionEconomyClass.FINALE
    )
    modifier_samples = [
        mission_modifier_for_offer(
            'SHOP-MODIFIER-CHECK',
            1,
            MissionOffer(f'SC_MOD_{index}', MissionEconomyClass.STANDARD),
        )
    ...
    mission_modifier_valid = bool(
        ...
        and mission_modifier_for_offer(
            'SHOP-MODIFIER-CHECK',
            1,
            MissionOffer('SC_FINALE', MissionEconomyClass.FINALE),
        ) is None
    )
    ...
    state_round_trip_valid = bool(
        ...
        == (BuffPurchase(gi_buff_entry.reward_id, 1),)
        and restored_run.starting_unit_ids == ('RLR_T1_INFANTRY',)
