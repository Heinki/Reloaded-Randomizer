...
from randomizer.config.game_profile import OBJECTIVE_REWARDS_READY
...
class AdvancedSettingsController:
    ...
    def advanced_faction_rank():
        return {
            faction: index for index, faction in enumerate(
                (*FACTION_ORDER, 'Neutral', 'Other')
            )
        }
    ...
    def advanced_unit_pool_entries(self):
        ...
        faction_rank = self.advanced_faction_rank()
        return sorted(
            entries.values(),
            key=lambda entry: (
                faction_rank.get(entry['faction'], len(faction_rank)),
    ...
    def advanced_buff_unit_is_visible(self, entry, included_only=True):
        ...
        return (
            ...
            in campaign_filter_factions(selected_campaign)
        )
    ...
    def refresh_advanced_pool_views(self, pool_key=None):
        ...
        def visible_for_campaign(entry):
            ...
            if arsenal_mode:
                return (
                    'Neutral' in entry_factions
                    or bool(entry_factions.intersection(arsenal_factions))
                )
            if selected_campaign == CAMPAIGN_FILTERS[0]:
                return True
            return (
                'Neutral' in entry_factions
                or bool(entry_factions.intersection(
    ...
    def advanced_pool_group_choices(self, pool_key):
        ...
        choices = []
        for faction in (*FACTION_ORDER, 'Neutral', 'Other'):
    ...
    def refresh_setting_states(self):
        ...
        if not OBJECTIVE_REWARDS_READY:
            ...
            self.rewards_on_victory_only_check.configure(
                state='disabled',
                text='Victory-only rewards (objective hooks pending)',
            )
