...
from randomizer.generation.state_controller import GenerationSettings
...
class StateController(GenerationSettings):
    ...
    def migrate_state(self):
        ...
        # Undo the temporary Reloaded-only opt-in migration. Mental Omega's
        # enemy-reward model is enabled by its configured maximum and exposes
        # acquired effects through the Enemy Rewards cards.
        enemy_opt_in_reverted = 'enemy_scaling_opt_in_version' in self.state
        if isinstance(reward_settings, dict):
            ...
            normalized_enemy = normalize_enemy_scaling_settings(enemy_source)
            if (
                enemy_opt_in_reverted
                and normalized_enemy['maximum_total_buffs'] == 0
            ):
                configured_enemy = normalize_enemy_scaling_settings(
                    self.config.get('generation', {}).get('enemy_scaling')
                )
                normalized_enemy = normalize_enemy_scaling_settings({
                    **normalized_enemy,
                    'maximum_total_buffs': configured_enemy[
                        'maximum_total_buffs'
                    ],
                })
            if reward_settings.get('enemy_scaling') != normalized_enemy:
                ...
                changed = True
        if enemy_opt_in_reverted:
            self.state.pop('enemy_scaling_opt_in_version', None)
            changed = True
        ...
        if 'mission_goal' not in self.state:
            ...
            changed = True
        if 'use_act_based_reward_multipliers' in self.state:
            self.state.pop('use_act_based_reward_multipliers', None)
            changed = True
        ...
        preserve_reward_history = (
            previous_schema in {16, 17, 19} and not discard_old_reward_history
        )
        ...
        enemy_plan_valid = (
            not enemy_opt_in_reverted
            and self.state.get('enemy_reward_plan_version')
            == ENEMY_REWARD_PLAN_VERSION
        ...
        if not enemy_plan_valid:
            ...
            changed = True
        if enemy_stack_model_changed or enemy_opt_in_reverted:
    ...
    @staticmethod
    def mission_reward_multipliers_enabled():
        """Finale bonuses are always active; Reloaded has no Act toggle."""
        return True
    ...
    def save_launcher_config(self, seed, mission_goal, rewards_per_check):
        ...
        self.config.pop('use_act_based_reward_multipliers', None)
    ...
    def save_settings_file(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile='C&C Reloaded Randomizer Settings.json',
            filetypes=(
