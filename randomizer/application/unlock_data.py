...
class UnlockDataController:
    ...
    def current_unlocks_text(self):
        ...
        if shared_groups:
            heading = 'Shared Unit Buffs'
            lines.append(heading)
            lines.append('=' * len(heading))
            lines.append(
                'Every pictured unit receives the bonuses listed beneath its group.'
            )
            lines.append('')
            for unit_ids, shared_group in sorted(
                ...
                lines.append(f'[[RLR_SHARED:{",".join(unit_ids)}]]')
