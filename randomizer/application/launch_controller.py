...
from randomizer.maps.generated import assert_file_hash
...
class LaunchController:
    ...
    def read_spawn_game_speed(self):
        for path in (SPAWN_INI, OPTIONS_INI):
    ...
    def cleanup_generated_root_maps(self):
        for path in GAME_ROOT.glob(f'{GENERATED_HOOK_PREFIX}_*.MAP'):
            if not is_generated_hooked_map(path):
                continue
            try:
                path.unlink()
            except OSError as exc:
                if callable(self.__dict__.get('append_log')):
                    self.append_log(
                        f'Could not remove generated map {path.name}: {exc}',
                        error=True,
                    )
    ...
    def extract_campaign_map(self, scenario):
        ...
        # Reloaded Battle.ini scenarios include nested campaign directories.
        # Mental Omega mostly used flat archive member names, so its original
        # extraction path never needed these parents.
        output_path.parent.mkdir(parents=True, exist_ok=True)
        if output_path.exists():
            return output_path

        loose_root_map = resolve_installed_scenario(scenario)
        if loose_root_map.is_file():
        ...
        if not mix_paths:
            raise FileNotFoundError(f'No {ARCHIVE_GLOB} archives found.')
        ...
        if missing:
            detail = (
                f'Map {scenario.upper()} was not found in {ARCHIVE_GLOB}'
            )
            if skipped:
                detail += ': ' + '; '.join(skipped)
            log_event(
    ...
    def write_launch_options(self, difficulty_value, game_speed_value):
        try:
            written = []
            skipped = []
            for path in (OPTIONS_INI,):
                # Do not create option files that the installation does not
                # Do not create option files the installation does not use.
                if not path.exists():
                    continue
    ...
    def write_optional_phobos_tooltip_options(self):
        ...
        prepared = []
        targets = (
            (OPTIONS_INI, 'Phobos', 'ToolTipDescriptions', 'true'),
    ...
    def disable_generated_rules_for_client(self):
        for path in (RULES_INI, DISABLED_RULES_INI):
    ...
    def build_command(self):
        ...
        if not wine:
            raise FileNotFoundError(
                'Wine is required to launch C&C Reloaded on this platform.'
            )
        winepath = shutil.which('winepath')
        if not winepath:
            raise FileNotFoundError(
                'winepath is required to resolve the C&C Reloaded executable.'
            )
        ...
        if not resolved_path:
            raise RuntimeError(
                'Wine could not resolve the C&C Reloaded executable.'
            )
    ...
    def close_game_after_victory(self, process, hook):
        ...
        if sys.platform != 'win32':
            try:
                ...
                self.append_log(
                    'Closed the spawned Wine process group after victory.'
                )
            except OSError as exc:
                self.append_log(
    ...
    def poll_hook_log(self):
        ...
        if self.active_hook:
            ...
            source_path = self.active_hook.get('source_path')
            source_sha256 = self.active_hook.get('source_sha256')
            if source_path and source_sha256:
                try:
                    assert_file_hash(source_path, source_sha256)
                    self.append_log(
                        f'Original mission integrity verified after launch: '
                        f'{scenario}.'
                    )
                except (OSError, ValueError) as exc:
                    self.append_log(str(exc), error=True)
                    log_event(
                        'original_mission_integrity_failed',
                        level=logging.ERROR,
                        scenario=scenario,
                    ...
                    messagebox.showerror(
                        'Original Mission Integrity Failure',
                        str(exc),
                    )
    ...
    def launch_mission_async(self, mission, extra_rules=None, launch_note=''):
        if FOUNDATION_ONLY or not GAMEPLAY_CATALOGUES_READY:
            detail = (
                'Mission launching is disabled during the C&C Reloaded '
                'foundation port. Reloaded mission and unit safety data must '
                'replace inherited gameplay catalogues first.'
            )
    ...
    def prepare_mission_launch_files(
        ...
        try:
            # Loose generated global rules files can affect every scenario.
            # Keep rewards in launcher state until they have a safe map-local
            # injection path.
            self.disable_generated_rules_for_client()
            self.cleanup_generated_root_maps()
            claim_runtime_asset_lease()
            ...
            try:
                ...
                self.append_log(
                    'Generated mission preparation failed. Randomizer launches '
                    'stop instead of falling back to an unmodified mission.',
                    error=True,
                )
                ...
                if self.randomizer_launch_active():
                    raise
            if self.randomizer_launch_active() and not hook:
                raise RuntimeError(
            ...
            if hook and hook.get('root_map'):
                try:
                    ...
                    self.append_log(traceback.format_exc(), error=True)
            launch_scenario = (
                hook.get('launch_scenario', scenario) if hook else scenario
            )
            self.write_spawn_ini(
                launch_scenario,
                difficulty_value,
                game_speed_value,
            )
