"""Manifest-driven C&C Reloaded Archipelago world."""
...
class CncReloadedLocation(Location):
    game = GAME_NAME
...
class CncReloadedWebWorld(WebWorld):
    ...
    tutorials = [
        Tutorial(
            ...
            "Connect the embedded C&C Reloaded Randomizer client.",
            "English",
            "setup_en.md",
            "setup/en",
            ["C&C Reloaded Randomizer contributors"],
        )
    ]


class CncReloadedWorld(World):
    """Full catalogue; each seed's shape comes from one signed manifest."""

    game = GAME_NAME
    web = CncReloadedWebWorld()
    options_dataclass = CncReloadedOptions
    options: CncReloadedOptions
    ...
    def create_item(self, name: str) -> CncReloadedItem:
        ...
        return CncReloadedItem(name, classification, item_id, self.player)
    ...
    def create_regions(self) -> None:
        ...
        for code in self.run_manifest["mission_order"]:
            ...
            region.add_locations(active, CncReloadedLocation)
            by_location.update({location.name: location for location in region.locations})
            local_victory = CncReloadedLocation(
                self.player,
    ...
    def _create_shop_regions(self, menu, victory, regions):
        ...
        victory.place_locked_item(CncReloadedItem(
            VICTORY_EVENT,
            ItemClassification.progression,
            None,
            self.player,
